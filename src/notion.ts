...
function readFocusWeekNumber(props: Record<string, unknown>): number | null {
  ...
  if (dbKey && SMALL_DBS.has(dbKey)) {
    try {
      const allRes = await withRetry("findRecordByTitle.fetchAll", () =>
        client.dataSources.query({ data_source_id: dsId(dbId), page_size: 100 }),
      );
...
async function setFounderFocus(entry: FounderFocusEntry): Promise<void> {
  ...
  const activeWeek =
    activeRow && "properties" in activeRow
      ? readFocusWeekNumber(activeRow.properties as Record<string, unknown>)
      : undefined;
...
async function getFounderFocusForWeek(weekNumber: number): Promise<FounderFocusEntry[]> {
  ...
  do {
    ...
    for (const row of res.results) {
      ...
      if (readFocusWeekNumber(props) !== weekNumber) continue;
...
async function getAllSupplierContacts(): Promise<PartnerContact[]> {
  ...
  let cursor: string | undefined;
  do {
    const res = await withRetry("getAllSupplierContacts", () =>
      client.dataSources.query({
...
async function getDueReminders(): Promise<ReminderRow[]> {
  ...
  do {
    ...
    for (const row of res.results) {
      if (!("properties" in row)) continue;
      const props = row.properties as Record<string, unknown>;
      const paraQuem = readMultiSelectNames(props["Para quem"]).filter(
        (n): n is FounderName => n === "Madalena" || n === "Mafalda" || n === "Beatriz",
      );
      if (paraQuem.length === 0) continue;
...
async function createSupplier(
  ...
  owner: OwnerValue,
  originalMsg: string,
  canalContacto?: "Instagram DM" | "Email" | "WhatsApp" | "Outro",
  ultimoContacto?: string | null,
  status: SupplierStatus = "A avaliar",
  email?: string | null,
): Promise<string> {
  if (!NOTION_SUPPLIER_DB_ID) {
    throw new Error("NOTION_SUPPLIER_DB_ID not set");
  }
  const page = await withRetry("createSupplier", () =>
    client.pages.create({
  ...
  await withRetry("createSupplier.sections", () =>
    client.blocks.children.append({
      block_id: page.id,
      children: [
...
async function updateSupplierFields(
  ...
  await withRetry("updateSupplierFields", () =>
    client.pages.update({
      page_id: pageId,
      properties: properties as Parameters<typeof client.pages.update>[0]["properties"],
    }),
...
async function createLead(
  ...
  const page = await withRetry("createLead", () =>
    client.pages.create({
...
async function updateLeadDetails(pageId: string, opts: CreateLeadOptions): Promise<void> {
  const properties: Record<string, unknown> = {};
  if (opts.telefone) properties["Telefone"] = { phone_number: opts.telefone };
...
async function createChurnFlag(
  ...
  const page = await withRetry("createChurnFlag", () =>
    client.pages.create({
...
async function updateChurnFlag(
  pageId: string,
  sinais: ChurnSignalType[],
  detalhes: string,
): Promise<void> {
  await withRetry("updateChurnFlag", () =>
    client.pages.update({
      page_id: pageId,
      properties: {
        Sinais: { multi_select: sinais.map((s) => ({ name: s })) },
 ...
 */
async function replacePageSection(pageId: string, content: string, section: string): Promise<void> {
  const targetId = await getOrCreateToggleId(pageId, section);
  const existing = await withRetry("listSectionChildren", () =>
    client.blocks.children.list({ block_id: targetId, page_size: 100 }),
  );
  ...
  const blocks = contentToBlocks(content);
  if (blocks.length) {
    await withRetry("replaceSection", () =>
      client.blocks.children.append({
        block_id: targetId,
...
async function getEntitiesForOwner(
  ...
  try {
    const res = await client.dataSources.query({
      data_source_id: dsId(dbId),
      filter: ownerFilter as Parameters<typeof client.dataSources.query>[0]["filter"],
      page_size: 20,
    });
    return res.results
      .filter((r) => "properties" in r)
