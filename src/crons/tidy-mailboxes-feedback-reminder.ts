...
function countPendingEntries(): number {
  const text = readFileSync(
    new URL("../knowledge-base/tidy-mailboxes-feedback-log.md", import.meta.url),
    "utf8",
  );
...
function buildMessage(count: number): string {
  ...
  return (
    `📋 Lembrete mensal: há ${count} ${noun} pending review em ` +
