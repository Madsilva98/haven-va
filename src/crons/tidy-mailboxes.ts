...
function isTagFresh(lastModifiedDateTime: string): boolean {
...
function isFromOwnDomain(mailbox: string, fromEmail: string): boolean {
  const at = mailbox.indexOf("@");
  if (at === -1) return false; // "me" — not a domain-based shared mailbox, never the sender-check target
