...
const PRIORITY_RANK: Record<string, number> = { "1. Alta": 0, "2. Média": 1, "3. Baixa": 2 };
...
export function trafficLight(task: OpenTask): TrafficLight {
  ...
  if (task.priority === "1. Alta" || task.deadline === today) return "red";
  if (task.deadline) {
    ...
    if (task.priority === "2. Média" || daysUntil <= 3) return "yellow";
