...
export function isBatch(intents: Intent[]): boolean {
...
export function batchKeyboard(batchId: string): InlineKeyboard {
...
export function storeBatch(intents: Intent[]): string {
  const id = `b${Date.now().toString(36)}`;
  batches.set(id, intents);
  while (batches.size > MAX_BATCHES) {
    const first = batches.keys().next().value;
    if (first !== undefined) batches.delete(first);
  }
  return id;
}

export function popBatch(id: string): Intent[] | null {
...
export async function postBatchCard(
  ctx: Context,
  intents: Intent[],
): Promise<void> {
