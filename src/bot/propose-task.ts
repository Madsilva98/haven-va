...
import type { ChatContext, NewTaskIntent, Priority } from "../types.js";
...
export async function proposeNewTask(
  ...
  const priority: Priority = intent.priority ?? "Média";
  ...
  try {
    createdPageId = await notion.createTask(
      extraction,
      priority,
      chatCtx.text,
      chatCtx.sender,
  ...
  record("confirmed", chatCtx.text, chatCtx.sender, extraction, `auto:${priority}`).catch(
    (err) => log.warn("propose_task.record_failed", { err: String(err) }),
  );

  const ownerLabel =
    intent.owner === "Unassigned" || !ownerId
  ...
  commitPending(sent.message_id, createdPageId).catch(
    (err) => log.warn("propose_task.commit_failed", { err: String(err) }),
  );
}
