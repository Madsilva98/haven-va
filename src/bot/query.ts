...
const QUERY_RE =
  /^(quais?|diz[- ]me|mostra|lista|quantas?|quem (tem|está|ficou|fica)|há |tens?|tenho|como (está|estão)|o que (está|temos|há)|qual|quando (é|são|temos))\b|\?$/iu;
  ...
  try {
    ...
    if (tasks.length > 0) {
      ...
      for (const t of tasks) {
        ...
        lines.push(
          `  - [${t.area}] "${t.title}" — ${t.owner}, ${t.status}${deadline}`,
        );
      }
    } else {
