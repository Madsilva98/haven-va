...
const COMPOUND_TASK_RE =
  /\b(reminder|lembrete|avisa|aviso|todos\s+os\s+dias|toda\s+a\s+semana|todo\s+o\s+m[eê]s|diariamente|semanalmente|mensalmente|no\s+projeto|ao\s+projeto|do\s+projeto|no\s+parceiro|ao\s+parceiro|no\s+evento|ao\s+evento|ao\s+influencer|prioridade\s+alta|urgente)\b/i;
...
async function handleEntityCommand(
  ...
  try {
    ...
    const taskGroups = await Promise.all(
      capped.map((e) => notion.getTasksForEntity(entityField, e.id)),
    );

    const lines: string[] = [`*${escapeMd(label)} — ${escapeMd(sender)}*`];
...
export async function handlePartners(ctx: Context): Promise<void> {
...
export async function handleInfluencers(ctx: Context): Promise<void> {
...
export async function handleCalendar(ctx: Context): Promise<void> {
  ...
  try {
    ...
    const lines: string[] = ["*calendário — próximos 3 dias*"];
...
export async function handleContent(ctx: Context): Promise<void> {
  try {
    ...
    const lines: string[] = ["*content — próximos 3 dias*"];
