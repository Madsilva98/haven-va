...
import type { ChatContext, OpenTask, Priority, SetDependencyIntent } from "../types.js";

function findTask(tasks: OpenTask[], title: string): OpenTask | undefined {
  const needle = title.toLowerCase();
  return tasks.find((t) => t.title.toLowerCase().includes(needle));
}

export async function handleSetDependency(
  ctx: Context,
  chatCtx: ChatContext,
  intent: SetDependencyIntent,
): Promise<void> {
  try {
    ...
    await ctx.reply(
      ...
      `Quando "${intent.prerequisite}" ficar feita, vou avisar ${intent.blockedOwner === "Unassigned" ? "o owner" : intent.blockedOwner}.`,
      { parse_mode: "Markdown" },
    );
  } catch (err) {
...
export async function checkAndUnblockDependents(
      ...
      try {
        ...
        if (task.owner !== "Unassigned") {
          ...
          if (telegramId) {
            await sendDM(
              telegramId,
              `✅ ${completedTitle} está feita — "${task.title}" está agora desbloqueada!`,
            );
          }
        }
      } catch (err) {
