...
export interface FilterableMessage {
  ...
  hasNonTextMedia: boolean;
}

export function shouldProcess(message: FilterableMessage): boolean {
  if (message.hasNonTextMedia) return false;
  if (message.isReplyToBot) return false;
  ...
  if (!REGEX_KEYWORDS.test(trimmed)) return false;
