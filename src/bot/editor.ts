...
const RECORD_EDIT_TOOL: Anthropic.Tool = {
  name: "record_edit",
  description:
    "Record an edit to an existing open task. Only call when the message clearly references one of the listed open tasks. If unsure, do not call this tool.",
  input_schema: {
    type: "object",
    properties: {
      targetTaskId: {
...
function compactOpenTasks(tasks: OpenTask[]): Array<Pick<OpenTask, "id" | "title" | "owner" | "area" | "priority" | "deadline" | "status">> {
  return tasks.map((t) => ({
    id: t.id,
    title: t.title,
    owner: t.owner,
...
function validate(
  ...
  if (typeof targetTitle !== "string") return null;
  if (
    typeof field !== "string" ||
...
export async function extractEdit(
  ctx: ChatContext,
): Promise<EditExtraction | null> {
