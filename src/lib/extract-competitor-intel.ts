...
const SYSTEM_INSTRUCTION =
  "Analisas newsletters de concorrentes e de negócios de inspiração para um estúdio de Pilates (The Haven), " +
...
export interface CompetitorIntelExtraction {
  tipo: string;
  resumo: string;
}
...
export async function extractCompetitorIntel(params: {
  ...
  subject: string;
  body: string;
}): Promise<CompetitorIntelExtraction[]> {
  const c = getClient();
  if (!c) throw new Error("extract_competitor_intel: sem ANTHROPIC_API_KEY");
  ...
  const findings: CompetitorIntelExtraction[] = [];
  for (const item of parsed) {
    if (
      item &&
      typeof item === "object" &&
      typeof (item as Record<string, unknown>).tipo === "string" &&
      typeof (item as Record<string, unknown>).resumo === "string"
    ) {
      findings.push({
        tipo: (item as { tipo: string }).tipo.trim(),
        resumo: (item as { resumo: string }).resumo.trim(),
      });
    }
  }
  return findings;
