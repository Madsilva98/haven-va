...
const dataSourceIdByDatabaseId = new Map<string, string>();

let initialized = false;

export async function initializeDataSources(
  client: Client,
  databaseIds: Array<string | undefined>,
): Promise<void> {
  const unique = Array.from(
    new Set(databaseIds.filter((id): id is string => typeof id === "string" && id.length > 0)),
  );
  ...
  for (const databaseId of unique) {
    ...
    const dataSources = (db as unknown as {
    ...
    if (dataSources.length > 1) {
      ...
      throw new Error(
        `Database ${databaseId} has ${dataSources.length} data sources ` +
...
export function dsId(databaseId: string): string {
  if (!initialized) {
    throw new Error(
      `dsId(${databaseId}) called before initializeDataSources(). ` +
  ...
  if (!resolved) {
    throw new Error(
      `data_source_id not resolved for database ${databaseId}. ` +
