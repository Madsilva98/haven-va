 ...
 */
export async function fetchAllVisitHistory(): Promise<Map<string, VisitHistory>> {
  if (!isStudioDbAvailable()) {
...
export function findVisitHistory(email: string | null, activity: Map<string, VisitHistory>): VisitHistory | null {
  if (!email) return null;
