...
function focusAskKeyboard(pageId) {
    return {
        inline_keyboard: [
            [
...
export async function runFocusCumpridoAsk(now = new Date()) {
...
export async function runFocusRollover(now = new Date()) {
    const currentWeek = weekOfYear(now);
    const active = await notion.getActiveFounderFocuses();
    let closed = 0;
    for (const entry of active) {
        if (entry.weekNumber === currentWeek)
            continue; // already rolled over (tapped Sim/Não)
        await rolloverAndAsk(entry.founder, currentWeek);
        closed++;
    }
    log.info("cron.focus_cycle.rolled_over", { closed });
}
async function rolloverAndAsk(founder, nextWeek) {
