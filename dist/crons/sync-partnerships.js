...
function setMessageCheckpoint(state, msgKey, classification, matchBasis, pipeline, notionPageId) {
...
async function forwardAndArchiveIfConfigured(msg) {
    ...
    try {
        ...
        if (!isSelfForward) {
            await outlook.forwardMessage(msg.mailbox, msg.id, [forwardTo], "Reencaminhado automaticamente — registado no Partner/Influencer Pipeline.");
        }
...
async function processMessage(msg, ownDomains, partnerLookups, influencerLookups, state) {
    ...
    try {
        if (classification === "parceiro") {
            const pageId = await notion.createPartner(externalParty.name, "Unassigned", formatOrigem(msg), "Parceria", "A contactar", msg.receivedDateTime);
        ...
        const pageId = await notion.createInfluencer(externalParty.name, "Unassigned", formatOrigem(msg), "Email", msg.receivedDateTime, "A contactar", externalParty.email || null);
...
export async function run() {
    ...
    for (const mailbox of mailboxes) {
        ...
        for (const msg of messages) {
            if (maxSeen === null || msg.receivedDateTime > maxSeen)
                maxSeen = msg.receivedDateTime;
            ...
            if (outcome.ok) {
                ...
                if (outcome.result === "updated_known")
                    updatedKnown++;
                else if (outcome.result === "created_partner")
                    createdPartners++;
                else if (outcome.result === "created_influencer")
                    createdInfluencers++;
                else if (outcome.result === "duplicate_skipped")
                    skippedDuplicates++;
