 ...
 */
export function computeChurnFlags(subscribers, bookings, failedPayments, pausedCycles, now) {
    ...
    const pausedCyclesByEmail = new Map();
    for (const c of pausedCycles) {
        ...
        if ((c.end.getTime() - c.start.getTime()) / 86_400_000 <= PAUSE_STRETCH_THRESHOLD_DAYS)
            continue;
    ...
    for (const [email, sub] of activeByEmail) {
        ...
        const mostRecentPauseEnd = pausedWindows
            ...
            .reduce((latest, end) => (!latest || end > latest ? end : latest), null);
        const stretchStart = mostRecentPauseEnd && mostRecentPauseEnd > sub.subscriptionStartsAt
            ? mostRecentPauseEnd
        ...
        if (tenureDays >= NO_BOOKING_GAP_DAYS) {
            const bookingsThisStretch = subBookings.filter((b) => b.status === "Booked" && b.bookingDate.getTime() <= nowMs && b.bookingDate >= stretchStart);
        ...
        if (allowance !== null) {
            ...
            const monthStats = months.map(({ start, end }) => {
                ...
                if (pausedWindows.some((w) => w.start < end && w.end > start))
                    return null; // paused for some/all of this month
...
export async function fetchChurnFlags(now = new Date()) {
    ...
    const pausedCycles = membershipRows
