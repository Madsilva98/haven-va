...
const lastSourceByChat = new Map();
function formatDatePt(iso) {
    const [y, m, d] = iso.slice(0, 10).split("-");
    return `${d}/${m}/${y}`;
}
export function formatSourceLine(views, asOf) {
    const base = `Fonte: ${views.join(", ")}`;
    return asOf ? `${base} · dados até ${formatDatePt(asOf)}` : base;
}
export function extractViewsFromText(text) {
...
export function rememberSource(chatId, views) {
    if (views.length > 0)
        lastSourceByChat.set(chatId, views);
}
export function lastSource(chatId) {
 ...
 */
export async function sendGroupMessageWithSource(text, views, asOf, pendingNote) {
