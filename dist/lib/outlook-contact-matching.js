...
function nameFrom(party) {
    ...
    if (name)
        return name;
    if (party.email)
        return domainToName(party.email);
    return "(desconhecido)";
}
/**
 ...
 */
export function buildContactLookups(contacts, ownDomains) {
...
function lookupContact(email, lookups) {
    if (!email)
        return null;
    const lower = email.trim().toLowerCase();
    const exact = lookups.emailToContact.get(lower);
    if (exact)
        return { contact: exact, matchBasis: "exact_email" };
    const byDomain = lookups.domainToContact.get(domainOf(lower));
    if (byDomain)
        return { contact: byDomain, matchBasis: "domain" };
    return null;
}
/**
 ...
 */
export function matchKnownContact(from, to, ownDomains, lookups) {
