...
let _labelCache = { data: new Map(), ts: 0 };
...
export function isAuthenticated() {
    if (process.env.GOOGLE_REFRESH_TOKEN_GMAIL_COMPETITOR)
        return true;
...
export async function getOrCreateLabel(name) {
    const gmail = await getGmailClient();
    if (!gmail)
        return null;
    const map = await getLabelMap(gmail);
 ...
 */
export async function listInboxMessagesFromSenders(senders) {
    if (senders.length === 0)
        return [];
    const gmail = await getGmailClient();
    if (!gmail)
 ...
 */
export async function listMessagesByLabel(labelName, excludeLabelName) {
    const gmail = await getGmailClient();
    if (!gmail)
        return [];
...
export async function tagAndArchive(messageId, labelId) {
    const gmail = await getGmailClient();
    if (!gmail)
...
export async function applyLabel(messageId, labelId) {
    const gmail = await getGmailClient();
    if (!gmail)
        throw new Error("gmail: not authenticated");
    await gmail.users.messages.modify({
