...
const wizardByUser = new Map();
function gcStale() {
    const now = Date.now();
    for (const [userId, st] of wizardByUser) {
        if (now - st.startedAt > STATE_TTL_MS)
...
export function isWeekCallback(data) {
    return data.startsWith("week:");
}
export function isAwaitingFocusFor(userId) {
...
export async function handleWeek(ctx, founder) {
    ...
    if (tasks.length === 0) {
        await ctx.reply("ainda não tens tasks no backlog. cria algumas primeiro.");
        return;
    }
    ...
    if (userId)
        wizardByUser.set(userId, state);
}
export async function handleWeekCallback(ctx, founder) {
    ...
    if (data === "week:done") {
        ...
        try {
            ...
            const writes = allOpen.map((t) => notion.setWeeklyPriority(t.id, state.selected.has(t.id)));
        ...
        try {
            await ctx.editMessageText(`prioridades guardadas (${state.selected.size})${suffix}. qual o teu foco operacional desta semana? (uma frase)`);
        }
        catch {
            await ctx.reply(`prioridades guardadas (${state.selected.size})${suffix}. qual o teu foco operacional desta semana? (uma frase)`);
        }
        return;
    }
    await ctx.answerCallbackQuery();
}
export async function handleWeekTextStep(ctx, founder, text) {
    const userId = ctx.from?.id;
    if (!userId)
        return;
    const state = wizardByUser.get(userId);
    if (!state || !state.awaitingFocus)
        return;
    ...
    try {
        ...
        await ctx.reply("não consegui guardar o foco — tenta outra vez com /focus <texto>");
        wizardByUser.delete(userId);
        return;
    }
    wizardByUser.delete(userId);
