...
export async function handleWeekCallback(ctx, founder) {
    ...
    if (data === "week:done") {
        ...
        try {
            await ctx.editMessageText(`prioridades guardadas (${state.selected.size}). qual o teu foco operacional desta semana? (uma frase)`);
        }
        catch {
            await ctx.reply(`prioridades guardadas (${state.selected.size}). qual o teu foco operacional desta semana? (uma frase)`);
        }
        return;
    }
    await ctx.answerCallbackQuery();
