...
export async function handleEditPending(tgCtx, chatCtx, intent) {
    ...
    if (!target.notionPageId) {
        ...
        await tgCtx.reply("ainda não posso editar isso — confirma a proposta primeiro");
        return;
    }
    await applyTaskEdit(tgCtx, target, intent.field, intent.value);
}
function resolveTarget(intent, buffer) {
...
async function applyCancel(tgCtx, target) {
...
async function applyTaskEdit(tgCtx, target, field, value) {
    if (!target.notionPageId || value === null)
        return;
    switch (field) {
        case "owner":
