...
const SYSTEM_INSTRUCTION = "És uma assistente de operações em pt-PT que escreve follow-ups breves para parceiros e influencers de um estúdio de pilates. " +
...
function fallback(name, daysSince) {
    return `queres seguir-up com ${name}? já passaram ${daysSince} dias do último contacto.`;
}
function buildUserPrompt(kind, row, daysSince) {
    const tipo = kind === "partner" ? "parceiro" : "influencer";
    const ultimoTopico = row.proximoPasso && row.proximoPasso.trim().length > 0
...
export async function draftFollowup(kind, row, daysSince) {
    const c = getClient();
    if (!c)
