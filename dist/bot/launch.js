...
import { generateTasksFromTemplate, } from "./templates.js";
...
export function launchKeyboard() {
 ...
 */
export async function proposeLaunch(ctx, extraction, originalMsg, sender) {
    const tasks = generateTasksFromTemplate(extraction);
    const proposal = formatLaunchProposal(extraction, tasks.length, tasks.map((t) => t.title));
 ...
 */
export async function handleLaunchCallback(ctx) {
    ...
    try {
        ...
        for (const task of entry.tasks) {
            ...
            const pageId = await notion.createTask(newTask, task.priority, entry.originalMsg, entry.originalSender);
            try {
        ...
        try {
            await ctx.editMessageText(`✅ ${created} tasks criadas para o lançamento de ${entry.extraction.name}`);
        }
        catch {
            // ignore
        }
