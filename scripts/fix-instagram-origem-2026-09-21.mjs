 ...
 * src/crons/leads-instagram-scan.ts (2026-09-21) wrote an unusable
 * internal id into the Origem field whenever a contact had no username —
 * looks like it might be a usable handle, isn't (not searchable/linkable;
 * for historical manual_upload contacts it was a synthetic "upload:Name"
 * id, not even a real Instagram id). Fixed in the cron itself same day;
 * this patches the ~260 pages already created before that fix, using the
 * checkpoint file (data/instagram-leads-sync-state.json) as the
 * authoritative list of exactly which pages this run touched.
 *
 * Read-then-conditionally-write: only pages whose Origem still matches the
 * old "Instagram DM · <handle-slot> · N mensagens · contacto <uuid>"
 * pattern with a non-"@" handle slot get patched — safe to re-run.
 *
