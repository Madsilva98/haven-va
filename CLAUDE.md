...
| `leads-reconcile.ts` | 08:10 Mon | Cleans up "Leads a contactar" before the scans below add anything new: archives any row the founder marked `Perdido`, and archives any still-open row whose email now has a real purchase on file (`src/lib/leads.ts` `hasRealPurchase`) — founder's call, a converted lead is archived outright, not left as a "Convertido" row. Channel-agnostic, works the same for every `Canal`. |
