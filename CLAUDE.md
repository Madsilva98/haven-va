...
| `leads-intro-pack.ts` | 08:20 Mon | Flags people whose intro pack (2 Classes / 10-Day Unlimited) ended 21+ days ago with no conversion since (`src/lib/intro-pack-conversion.ts`) — the day-21 threshold and "no pack-type split needed" were both empirically validated, not assumed. Writes into the same "Leads a contactar" DB, `Canal = "Intro Pack"`. Dedup uses `notion.findLeadByEmailAny(email, "Intro Pack")` (not the open-only `findLeadByEmail`) so a `Perdido` row still blocks recreation — see the `leads-reconcile.ts` exception above. Scoped to the Intro Pack channel specifically: an unscoped match would let a not-yet-archived closed lead on a different channel (Email/WhatsApp/Instagram) silently block creation of a legitimate new Intro Pack lead for the same email. Depends on Studio Supabase (configured in production, see the `birthdays.ts` note above). |
