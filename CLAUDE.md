...
| `leads-intro-pack.ts` | 08:20 Mon | Flags people whose intro pack (2 Classes / 10-Day Unlimited) ended 21+ days ago with no conversion since (`src/lib/intro-pack-conversion.ts`) — the day-21 threshold and "no pack-type split needed" were both empirically validated, not assumed. Writes into the same "Leads a contactar" DB, `Canal = "Intro Pack"`. Dedup uses `notion.findLeadByEmailAny` (not the open-only `findLeadByEmail`) so a `Perdido` row still blocks recreation — see the `leads-reconcile.ts` exception above. Depends on Studio Supabase (configured in production, see the `birthdays.ts` note above). |
