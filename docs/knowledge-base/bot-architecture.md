...
**Notion cleanup still pending (2026-09-16):** the Backlog DB's `Depende de` relation property and the `Bloqueado` option on its `Status` select field are now unused by any code but haven't been deleted from the live Notion database itself — that's a manual/API step against production schema, done separately once deemed safe (see `notion-api-gotchas.md` for why schema mutations here get extra care).
