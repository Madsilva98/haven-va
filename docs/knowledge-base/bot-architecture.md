...
| `crons/birthdays.ts` | `0 8 * * *` | no | Birthday reminders, today-only (was a 7-day-ahead preview until 2026-09-15). **Currently a silent no-op in production** — `STUDIO_SUPABASE_URL`/`STUDIO_SUPABASE_KEY` are unset, confirmed via NAS logs (`studio_supabase.disabled`). Needs real Studio Supabase credentials added to the NAS `.env` before this cron does anything. |
...
| 8 | Birthday digest | 08:00 daily | **Studio Supabase `kenko_customers`**: every row with a non-null `date_of_birth`, filtered client-side for a birthday landing **today** (`daysAhead=0` — was 7 days, simplified 2026-09-15, founder's call: "just send a message at 8am that day, nothing else"). ⚠️ **Confirmed broken in production 2026-09-15**: `STUDIO_SUPABASE_URL`/`STUDIO_SUPABASE_KEY` are not set on the NAS (`studio_supabase.disabled` in the container logs at every boot) — `fetchUpcomingBirthdays` returns `[]` unconditionally, so this cron has silently never sent anything. Needs real Studio Supabase credentials added to the NAS `.env` (see `deploy-and-access.md`) to actually do anything; the code itself is correct and was verified with tests. | Group | Silent (nothing sent) when nobody has a birthday today | `src/crons/birthdays.ts` |
| 9 | Founder Focus Sunday ask ("cumpriste?" Sim/Não) | 18:00 Sun | **Founder Focus DB**: `Ativo = true` rows whose `Semana` equals the current ISO week (i.e. hasn't been closed yet). | DM | Whoever's active Founder Focus row is this week and still unanswered | `src/crons/founder-focus-cycle.ts` (`runSundayAsk`) |
| 10 | Founder Focus Monday fallback (closes week + asks new goals) | 08:00 Mon | **Founder Focus DB**: `Ativo = true` rows whose `Semana` is still *last* week's (i.e. nobody tapped Sunday) — rolls the row over (deactivate + create/activate next week) before sending. | DM | Whoever didn't tap Sunday | `src/crons/founder-focus-cycle.ts` (`runMondayReask`) |
| 11 | Tidy-mailboxes feedback-log monthly nudge | 09:00, day 1 of month | **None** — pure schedule, no DB read; it's a nudge to check a markdown file, not a data digest. | DM | Hardcoded to **Madalena only** | `src/crons/tidy-mailboxes-feedback-reminder.ts` |
| 12 | Dependent task unblocked | event-driven — fires when a task with dependents is marked done via the assistant | **Backlog DB**: `Depende de` relation contains the just-completed task's id AND `Status = Bloqueado`. Only reachable through the assistant's `update_record` tool (`db=backlog`, `field=status`, `new_value=Feito`) — there is no slash command or button that sets a task to `Feito`, and a task marked done directly in Notion's UI does **not** trigger this (the bot doesn't watch Notion for changes). | DM | The owner of the now-unblocked dependent task | `src/bot/dependencies.ts:104`, called from `src/bot/assistant.ts:847` |
...
Note on #9/#10: once the DM is sent, the founder's "porquê?" explanation and "quais são os teus objetivos?" answer are captured as plain follow-up DMs in the same chat by `src/bot/focusCallbacks.ts` (`tryConsumeFocusComment`, `tryConsumeGoalsAnswer`) — not a separate proactive push.
...
5. **No overlap guard on the 5-minute reminders cron.** If one run is still iterating `due` when the next tick fires (slow Notion), both could send the same batch. Low likelihood, not yet hardened. **Not yet actioned.**
...
Items 1, 2, 3, 4, 6, 7, and 8 are resolved (fixed, removed, or confirmed a non-issue at current scale) and verified (`npm run typecheck`, `npm run test`, `npm run build` all clean throughout). Item 5 is still open — flagged per the review-gate, nothing else has been changed.
