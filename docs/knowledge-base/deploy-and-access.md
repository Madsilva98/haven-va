...
**Third incident, 2026-09-21**: while debugging why Node's `--env-file` flag wasn't picking up `STUDIO_SUPABASE_URL` from the real NAS `.env` (root cause: an orphaned/malformed secret-looking line sitting in the file with no `KEY=` prefix — likely leftover raw text from a past paste — which breaks Node's built-in env-file parser's multi-line/quote handling even though Docker Compose's own `env_file:` parsing tolerates it fine, which is why production itself was never affected), two commands used for diagnosis — `od -c` on the file's raw bytes, and an `awk '{print ... $1}'` with `FS="="` run across a range of lines — each printed a real secret into the session transcript: `TELEGRAM_BOT_TOKEN` (via `od -c`) and the Studio Supabase `anon` legacy key (via `awk`, because the orphaned line above has no `=`, so `awk` treated the entire line as field 1). Neither secret left the local session, but both were rotated same-day out of caution: `TELEGRAM_BOT_TOKEN` via BotFather `/revoke` (immediate, update `.env`, `docker compose up -d --force-recreate` right after — the bot is down in the gap, verified via `bot.identified` in logs afterward). The Studio Supabase legacy `anon` key rotation was deferred — the project also has a newer `sb_publishable_...` key already provisioned that haven-va isn't using yet, and Mafalda's separate Instagram/WhatsApp webhook tooling reads from the same Supabase project, possibly using the same legacy key — disabling/regenerating it without checking with her first could break her tooling. Lesson: **never inspect a secret-bearing file with a command that echoes raw content or fields**, even for a seemingly innocuous debugging purpose like "why isn't this env var loading" — use presence/length-only checks (`grep -c '^KEY=.\+' file`, or in Node, `process.env.KEY?.length`) exclusively, no exceptions, and never `od`/`head -c`/`awk '{print}'` a file that might contain secrets, full stop. The orphaned line itself in the NAS `.env` still needs manual cleanup — not attempted here, left for a founder to confirm what it actually is before removing it.
