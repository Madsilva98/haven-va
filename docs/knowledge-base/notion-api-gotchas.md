...
8. **`dataSources.update()` with `select: { options: [] }` WIPES the live option list — not a no-op.** Happened for real on 2026-09-07: `scripts/setup-notion-dbs.mjs` had `Lista: { select: { options: [] } }` as a "dynamic options, nothing to declare" placeholder for the Listas DB. Running the script deleted every real option, which cleared that property on all ~48 existing cards (Notion clears a page's select value the moment its referenced option is deleted). Only the ~20% of cards whose "Origem" text happened to name the list were recoverable, by hand. **Rule: never include a select/multi_select/status property in a schema-sync script's payload unless you intend to fully own and overwrite its option list. For a property whose options are created dynamically at runtime (by the bot writing new values), omit it from the script entirely** — `dataSources.update()` only touches properties you explicitly pass.
9. **A DB named "Status" is not necessarily Notion's `status` property type — check before assuming.** Found and fixed 2026-09-14: `Backlog`, `Partner Pipeline`, and `Influencer Pipeline`'s live "Status" columns are actually plain `select` properties (someone picked the wrong type in Notion's UI when creating them — `select` and `status` look near-identical in the picker), while `To Discuss`, `Decisions`, `Projects`, and `Events`' "Status" columns really are `status` type. The code assumed `status` everywhere. Real-world impact, confirmed via `scripts/test-data-sources-writes.mjs` against production: `createTask`, `createPartner`, and `createInfluencer` were failing outright (`"Status is expected to be select."`, a 400 `validation_error` — the whole page create was rejected, not just that field), and every `dataSources.query()` filter on those three DBs' Status (`getOpenTasks`, `getWeeklyPriorities`, `getWeeklyCompletedSince`, `getWeeklyOverdueTasks`, `getDependentTasks`, `getTasksForEntity`, `getPartnersStale`, `getInfluencersStale`) used the wrong filter operand shape too. Reads didn't error but silently returned wrong data — `readStatusName()` looks for a `status` key that was never there, so every task appeared with `status: "To do"` regardless of its real value. **Fix applied**: writes/filters for these three DBs now use `{ select: { ... } }` instead of `{ status: { ... } }`; reads go through a new combined `readStatusOrSelectName()` helper (tries `status` shape, falls back to `select`) so the same code works regardless of which type a given DB actually uses. **Before writing or filtering on ANY property whose name suggests a type (`Status`, `Date`, etc.), verify the live type** — `dataSources.retrieve({ data_source_id })` and inspect `.properties["Status"].type` directly; don't trust the property name or what `scripts/setup-notion-dbs.mjs` declares (that script's declared type doesn't force-migrate an existing property of a different type — it can silently mismatch the live schema for years). Verification command: `node --env-file=.env.local scripts/test-data-sources-writes.mjs` (create+archive, safe to run anytime) should show all creates passing.
...
- **`notion.ts`'s generic `updateRecord()` helper overwrites `rich_text` fields, it doesn't append.** There's no native "append to a rich_text property" — that only exists for page-body block sections (`appendToPageSection`/`createPartner`'s toggle headings), a completely different mechanism. Any caller that wants to add to an existing note (e.g. `scripts/apply-outlook-findings.mjs`, see [`outlook-partnerships-sync.md`](outlook-partnerships-sync.md)) must read the current value first and compose the concatenated text client-side.
...
2026-09-15 — Added a `"📓 Log"` toggle section to Partner Pipeline pages (pre-seeded in `createPartner`, lazily created on existing pages via `getOrCreateToggleId`'s existing fallback) for dated, append-only history entries, written both by `sync-partnerships`' apply step and by the live Telegram assistant. Confirmed by reading `contentToBlocks` directly (`src/notion.ts`) that it builds plain `{ type: "text", text: { content } }` runs with **no markdown parsing** — a `**bold**` string renders as literal asterisks in Notion, not bold. Worth remembering any time free text is about to be written into a page body block: this codebase's two content-writing paths are `richText()` (property values, chunks at 2000 chars) and `contentToBlocks()`/`appendToPageSection()` (body blocks, line-by-line, zero inline formatting support, and — separately — no chunking of an individual long line against the 2000-char rich_text limit either).

2026-09-15 — Audited `RECORD_DB_CONFIGS.partners.fields` in `src/notion.ts` against the live Partner Pipeline data source schema (`dataSources.retrieve()`) rather than trusting the code's existing assumptions — found it was missing 3 real properties: `Próximo passo` (rich_text), `Categoria` (select: Corporate/Eventos/Parceria), `Origem` (rich_text). Added all three as `proximoPasso`/`categoria`/`origem`. Motivated by wanting the Telegram assistant's `update_record` tool to be able to set "Próximo passo" for a partner from a DM/group message — it couldn't before, since `updateRecord()`'s generic dispatch only knows about fields present in this config, regardless of what the assistant's own tool schema claims to support. Verified the write for real against the PUANI test partner page (write, read back, restore original value) before shipping. `RECORD_DB_CONFIGS.influencers` has the same "Próximo passo" gap (confirmed via the same schema dump) but wasn't touched — out of scope for this pass, noted here so it isn't mistaken for an oversight.

2026-09-14 — Found and fixed a real production bug while building the Outlook partnerships sync: Backlog/Partner Pipeline/Influencer Pipeline's "Status" property is `select` in Notion, not `status` as the code assumed — `createTask`/`createPartner`/`createInfluencer` were failing outright, and several read paths silently showed wrong data. See TL;DR item 9. Also added the `updateRecord()` rich_text-overwrite-not-append gotcha, found in the same session (see [`outlook-partnerships-sync.md`](outlook-partnerships-sync.md)).
