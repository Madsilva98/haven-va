...
4. **Auto-archive fast path**: if the sender is an exact match in `DEFAULT_AUTO_ARCHIVE_SENDERS` (extend via `TIDY_MAILBOXES_AUTO_ARCHIVE_SENDERS`) — known pure logistics-notification addresses (couriers, Amazon's dedicated shipping-confirmation senders) — archives immediately. No invoice check, no Haiku call.
